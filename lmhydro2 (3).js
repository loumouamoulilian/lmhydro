...
function sizeHydraulicSection() {
  ...
  if (type === "circ") {
    ...
    result = {
      ...
      capacite: cap.q,
      surcapacité_pct: oversize,
      optimisation: oversize <= 25 ? "OK: surcapacité <= 25%" : "Diametre standard superieur: surcapacité > 25%, vérifier un diametre non standard ou une section rectangulaire.",
    ...
    result = {
      ...
      capacite: cap.q,
      surcapacité_pct: oversize,
      optimisation: oversize >= -0.5 && oversize <= 25 ? "OK: profondeur V optimisee, surcapacité <= 25%" : "Alerte: vérifier pente des parois, K ou pente collecteur.",
...
function updateSectionUI() {
  ...
  const dims = s.type === "circ"
    ? `Diametre ${s.dimensions.diametre_mm} mm`
    : s.type === "vditch"
  ...
  const squareDetail = s.dimensions.b_hydraulique_m
    ...
    : s.type === "vditch"
      ? `<div class="metric"><strong>z=${fmt(s.dimensions.pente_parois_z, 2)} | voile incline=${fmt(s.dimensions.longueur_voile_incline_m, 2)} m</strong><span>Section en V, profondeur et largeur en surface dimensionnees automatiquement</span></div>`
      : "";
  $("#heroSection").textContent = s.section_proposee;
  const oversizeClass = s.surcapacité_pct >= -0.5 && s.surcapacité_pct <= 25 ? "ok" : "critical";
  const oversizeSign = s.surcapacité_pct >= 0 ? "+" : "";
  $("#sectionResult").innerHTML = `
    ...
    <div class="metric ${oversizeClass}"><strong>${oversizeSign}${fmt(s.surcapacité_pct, 1)}%</strong><span>${s.optimisation}</span></div>
...
function calculateCircularSteel() {
  ...
  return finishSteelState({
    ...
    displayRows: [
      ...
      ["Sections acier", `Longitudinal requis ${Math.ceil(asLong)} mm2/m, cerces requis ${Math.ceil(asRing)} mm2/m`],
      ["Ferraillage", `Longitudinaux ${barLabel(longBar)}, cerces/etriers ${barLabel(ringBar)}`],
...
function calculateVSteel() {
  ...
  return finishSteelState({
    ...
    ferraillage_manuel: !!$("#manualSteel")?.checked,
    aciers_proposes_auto: { radier_longitudinal: barLabel(autoRadier), radier_transversal: barLabel(autoRadierDist), voiles_inclines: barLabel(autoVoile), repartition_voiles: barLabel(autoVoileDist) },
    acier: {
    ...
    displayRows: [
      ...
      ["Ferraillage", `Radier ${barLabel(radierMain)} + ${barLabel(radierDist)}, voiles inclines ${barLabel(wallVert)} + repartition ${barLabel(wallHoriz)}`],
...
function generateCircularPlan() {
  ...
  const svg = `
    ...
    <g transform="translate(610 118)">
      ...
      ${legendLine(18, 72, "Longitudinaux", st.buse_longitudinales || st.radier_longitudinal)}
      ${legendLine(18, 110, "Cerces/etriers", st.cerces || st.parois_horizontales)}
...
function generateVPlan() {
  ...
  const barColor = "#b1261e";
  const svg = `
    ${planHeader("LOUMOUAMOU HYDRO - Coupe ferraillage section en V", "Voiles inclines, fond renforce et armatures de repartition exportables DXF")}
